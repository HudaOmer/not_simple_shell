 ...
 */
int is_chain_del(data_t *data, char *buf, size_t *p)
{
	size_t j = *p;
...
/**
 * chain_cont - checks we should continue chaining based on last status
 * @data: the parameter struct
 * @buf: the char buffer
 * @p: address of current position in buf
 ...
 * Return: Void
 **/
void chain_cont(data_t *data, char *buf, size_t *p, size_t i, size_t len)
{
	size_t j = *p;
